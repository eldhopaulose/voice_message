...
class VoiceController extends MyTicker {
  ...
  Future startPlaying(String path) async {
    try {
      // Set the audio source based on whether it's a file or URL
      if (isFile) {
      ...
      // Set the position and playback rate
      await _player.seek(currentDuration);
  ...
  void _listenToPlayerState() {
    ...
    playerStateStream = _player.onPlayerComplete.listen((_) async {
      await _player.stop();
      currentDuration = Duration.zero;
      playStatus = PlayStatus.init;
      animController.reset();
  ...
  void changeSpeed() {
    ...
    _player.setPlaybackRate(
        speed.getSpeed); // Changed from setSpeed to setPlaybackRate
    _updateUi();
  }
  ...
  Future setMaxDuration(String path) async {
    try {
      // Set the appropriate source based on file type
      if (isFile) {
        await _player.setSource(DeviceFileSource(path));
      } else {
        await _player.setSource(UrlSource(path));
      ...
      // Get the duration after setting the source
